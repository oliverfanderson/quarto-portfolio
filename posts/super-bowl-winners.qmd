...
In celebration of the 2023 Super Bowl, I wanted explore the features of <i>gt</i> R package by creating a table of all the teams that had won the Super Bowl since Super Bowl I.
...
# format date in an aesthetically pleasing way, add title + subtitle
superbowls %>% 
  gt() %>% 
  tab_header(
  ...
  text_transform(
    ...
    locations = cells_body(c(win_logo_url)),
    fn = function(x) {
  ...
  text_transform(
    #Apply a function to a column
    locations = cells_body(c(lose_logo_url)),
    fn = function(x) {
      #Return an image of set dimensions
      web_image(
        url = x,
